...
import { normalizeFastFlag, RawFastFlag } from "./parser.js";
...
export class FastFlagScraper {
	...
	async seed(url: string, githubToken?: string): Promise<{ added: number; updated: number }> {
		...
		for (const raw of rawFlags) {
			...
			const existing = await this.store.get<any>(key);
	...
	private async fetchWithRetry(url: string, token?: string, attempt = 0): Promise<any> {
		try {
	...
	private ensureArray(data: any): RawFastFlag[] {
